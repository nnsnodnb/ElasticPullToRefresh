...
public class ElasticPullToRefresh: UIView, UIGestureRecognizerDelegate {
	...
	convenience public init(scrollView: UIScrollView) {
		self.init(frame: CGRect.zero, scrollView: scrollView)
	}
	...
	public init(frame: CGRect, scrollView: UIScrollView) {
		...
		let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(ElasticPullToRefresh.didPan(_:)))
	...
	private func updateContentInsets(_ insets: UIEdgeInsets) {
		isAnimating = true
